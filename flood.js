...
var namefirst = new Array("Abby", "Bob", "coding","daffy","Egypt","Frilly","Godly","Hacker","Italian","Jack","Kilo","Lemon","Monny","Note","Zilly","Yeet","Botted","Botted");
var namesecond = new Array("Hax", "Bob", "coding","daffy","Egypt","Frilly","Godly","Hacker","Italian","Jack","Kilo","Lemon","Monny","Note","Zilly","Yeet","Botted","Botted");
let i = 0,
botNumber = 0,
answer = "",
twoStepVar = "",
...
function joinKahoot(pin, name) {
    ...
    if (config.RandomName) {
      var rA = Math.floor(Math.random()*namefirst.length);
      var rB = Math.floor(Math.random()*namesecond.length);
      var rand = getRandomInt(1000,9999);
      name = namefirst[rA] + namesecond[rB] + rand;
    }
    ...
    bot.on("questionStart", question => {
      if (config.NaturalAnswerTime) {
      var randomwait = getRandomInt(1000,20000);
      }else{
      var randomwait = 0
      }
      setTimeout(function(){
        ...
        if (question.type == "word_cloud") {
            if (config.userChoice) {
                ...
                console.log(`${nameBack(question.client.name)} answered question ${question.index+1} out of ${question.quiz.questionCount}. It responded with 'Uhh hello im a bot'`);
                bot.answerQuestion('Uhh hello im a bot');
            }
